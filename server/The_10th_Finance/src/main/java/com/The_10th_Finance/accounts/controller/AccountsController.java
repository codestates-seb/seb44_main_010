...
public class AccountsController {
    ...
    @PostMapping("/post")
    public Response.SuccessResponse postAccount(@Valid @RequestBody AccountsPost accountsPost){
        Accounts accounts =accountsService.post(accountsMapper.accountsPostToAccounts(accountsPost));
        return  new Response.SuccessResponse<>(accountsMapper.accountsToAccountsResponseDto(accounts),HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public Response.SuccessResponse getOne(@PathVariable(name = "id")@Positive Long id){
        Accounts accounts = accountsService.getOne(id);
        return new Response.SuccessResponse<>(accountsMapper.accountsToAccountsResponseDto(accounts),HttpStatus.ACCEPTED);
    }
...
//    }
    ...
    @PatchMapping("/{id}")
    public Response.SuccessResponse patchAccount(@PathVariable(name = "id") @Positive Long id,
                                    @RequestBody AccountsPatch accountsPatch) {
        Accounts accounts = accountsService.getOne(id);
        accountsMapper.accountsToPatch(accountsPatch,accounts);
        return new Response.SuccessResponse<>(accountsMapper.accountsToAccountsResponseDto(accountsService.post(accounts)),HttpStatus.ACCEPTED);
    }
